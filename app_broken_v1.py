...
def calculate_retention_heatmap(sales_df):
    """Compute monthly cohort retention heatmap data."""
    if sales_df.empty or 'Client' not in sales_df.columns or 'Date' not in sales_df.columns:
        return None
    
    df = sales_df[['Client', 'Date']].copy()
    df['Month'] = df['Date'].dt.to_period('M')
    
    # 1. First purchase month per client (Cohort)
    df['Cohort'] = df.groupby('Client')['Month'].transform('min')
    
    # 2. Group by Cohort and Transaction Month
    cohort_counts = df.groupby(['Cohort', 'Month']).agg(n_customers=('Client', 'nunique')).reset_index()
    
    # 3. Period number (months since first purchase)
    def diff_month(row):
        return (row['Month'].year - row['Cohort'].year) * 12 + (row['Month'].month - row['Cohort'].month)
    
    cohort_counts['Period'] = cohort_counts.apply(diff_month, axis=1)
    
    # 4. Pivot and calculate %
    pivot = cohort_counts.pivot(index='Cohort', columns='Period', values='n_customers')
...
def compute_market_basket(sales_df, top_n=10):
    """Compute category affinity scores (Frequently Bought Together)."""
    if 'Date' not in sales_df.columns or 'Category' not in sales_df.columns:
        return None
        
    # Group by Client + Date to define a "Basket"
    baskets = sales_df.groupby(['Client', 'Date'])['Category'].apply(list).reset_index()
    baskets = baskets[baskets['Category'].map(len) > 1] # Only multi-item baskets
    ...
    for basket in baskets['Category']:
        # Sort to avoid (A, B) and (B, A) being different
        unique_cats = sorted(list(set(basket)))
    ...
    res = pd.DataFrame(counts.most_common(top_n), columns=['Pair', 'Frequency'])
    res['Pair_Name'] = res['Pair'].apply(lambda x: f"{x[0]} + {x[1]}")
    return res
